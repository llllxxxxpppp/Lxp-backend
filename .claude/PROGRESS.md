...
### 탈퇴 회원 이메일 마스킹 (2026-07-08)

`MEMBER.md` 서비스 비즈니스 규칙 - 공통: "탈퇴된 회원은 이메일 앞 3글자만 보여준다."를 반영. REPORT.md 7번 항목(탈퇴 회원 이메일 마스킹 미구현) 해소.

**구현 내용**
- `member/dto/response/UserResponseDTO.from(Member)` — `member`가 `RegularMember`이고 `getWithdrawnAt() != null`(탈퇴한 일반 회원)인 경우에만 이메일을 마스킹. 앞 3글자(`VISIBLE_EMAIL_PREFIX_LENGTH` 상수)는 유지하고 나머지는 전부 `*`로 치환(원본과 동일한 길이 유지)

**핵심 불변식**
- 마스킹 판단은 `RegularMember.getWithdrawnAt()`(탈퇴 일시) 기준으로만 하며, 강사 정지(soft delete, `deleted=true`)나 어드민은 마스킹 대상이 아님 — STEP1에서 정지/탈퇴 개념을 분리해둔 덕분에 정확히 구분 가능
- 이메일 길이가 3자 이하이면 마스킹 없이 원본 그대로 반환 (프로젝트 이메일 검증 정규식상 실질적으로 발생하기 어려운 엣지 케이스)

**테스트 현황**: `UserResponseDTOTest`(신규, 4개: 미탈퇴 회원/탈퇴 회원/정지된 강사/어드민 각각의 이메일 노출 여부) — 전체 통과, PMD 통과(매직넘버 `3`을 상수로 추출), 전체 회귀 테스트 통과

---

## REPORT.md 대응 결과 요약 (2026-07-08)

`REPORT.md`에서 발견된 회원 관리 바운디드 컨텍스트 불일치 7개 항목에 대한 최종 처리 결과:

1. 정지/탈퇴 개념 혼재 → **해소** (STEP1: `suspendedAt`/`withdrawnAt` 분리, 정지 시에도 로그인 차단을 위해 `deleted=true`로 통일)
2. 일반 회원 정지 기능 미구현 → **해소** (STEP1/STEP2: `RegularMember.suspend()` + `MemberSuspendedEvent` 발행)
3. 회원 신고 기능 미구현 → **사용자 결정에 따라 이번 범위에서 구현하지 않기로 확정** (미구현 상태 유지, `.claude/DO_NOT_IMPLEMENT.md`에도 별도로 명시됨)
4. 강사 정지 이벤트 미구현 → **부분 해소** (STEP2: `common/event`/`member/event` 인프라 구축 및 이벤트 발행까지 구현. 구독권/강좌 컨텍스트의 실제 리스너 처리는 아래 "후속 구현 필요" 참고)
5. 어드민 전용 강사 생성/정지 권한 검증 지점 없음 → **해소** (STEP3: `AdminMemberController` + `SecurityConfig` 권한 매칭 버그 수정)
6. 자기수정 유스케이스 미노출 → **부분 해소** (STEP4: 비밀번호 변경·강사 프로필 변경·본인 탈퇴는 API로 노출. 이메일 자기수정은 사용자 결정에 따라 구현하지 않기로 확정)
7. 탈퇴 회원 이메일 마스킹 미구현 → **해소** (STEP5)

**남은 후속 구현 필요 사항 (다른 바운디드 컨텍스트 담당, 순서대로)**
1. 구독권 관리 컨텍스트: `MemberSuspendedEvent`/`MemberWithdrawnEvent` 리스너 추가 후 기존 `SubscriptionService.suspendSubscription(memberId)`와 연결(정지 시), 탈퇴 시 구독권 추가 처리 로직 신규 구현
2. 강좌 컨텐츠 관리 컨텍스트: `InstructorSuspendedEvent` 리스너 추가 + 해당 강사의 강좌를 비공개 처리하는 기능 신규 구현
3. (회원 신고 기능 재개 시) 구독권 관리 컨텍스트에 "회원이 유효한 구독권을 보유 중인지" 조회하는 공개 메서드 추가 필요 — 단, 신고 기능 자체는 현재 구현 계획 없음

---

### 회원 관리 바운디드 컨텍스트 리팩토링 (2026-07-08)

STEP1~STEP5 작업 과정에서 `MemberService`/`MemberSelfController`에 쌓인 중복 코드를 정리 (기능 변경 없는 순수 리팩토링).

**구현 내용**
- `member/service/MemberService` — 반복되던 "조회 → 타입 검증 → 예외" 패턴을 다음 private 헬퍼로 추출: `ensureEmailNotTaken(String)`, `getMemberOrThrow(Long)`, `getRegularMemberOrThrow(Long)`, `getInstructorMemberOrThrow(Long)`. `register`/`registerInstructor`/`suspendMember`/`withdrawMember`/`suspendInstructor`/`changePassword`/`updateInstructorProfile` 전체가 이 헬퍼를 사용하도록 정리
- `member/controller/MemberSelfController` — `Long.parseLong(authentication.getName())` 반복 3곳을 `resolveMemberId(Authentication)` 헬퍼로 추출
- `member/model/entity/Member` — `updateEmail`/`updatePassword`/`markDeleted`가 각자 `this.updatedAt = OffsetDateTime.now()`를 직접 호출하던 것을 기존 `touch()` 메서드 호출로 통일
- `src/test/java/com/lcs/lxp/member/service/MemberServiceTest.java` — `UserResponseDTO` import 정리(전체 경로 표기 대신 import 사용)

**핵심 불변식**
- 예외 메시지 일부가 통일됨(예: "일반 회원만 정지시킬 수 있습니다." / "일반 회원만 탈퇴할 수 있습니다." → "일반 회원이 아닙니다."). 테스트는 `assertThrows(MemberException.class, ...)`로 타입만 검증하므로 동작상 영향 없음을 확인
- `changePassword`는 회원 유형 제한이 없어야 하므로 `getMemberOrThrow`(타입 무관)를 사용하고, `getRegularMemberOrThrow`/`getInstructorMemberOrThrow`는 사용하지 않음 — 의도된 설계
- 검토했지만 리팩토링하지 않기로 한 항목: `LoginRequestDTO`/`SignupRequestDTO`(필드 구성은 동일하나 로그인/가입이라는 다른 유스케이스라 통합하지 않음), `RegularMember.suspend()`/`InstructorMember.suspend()`(현재 구현은 동일하나 서로 다른 도메인 개념이라 상위 클래스로 끌어올리지 않음 — 어드민에게 불필요한 `suspend()`가 노출되는 것도 방지)

**테스트 현황**: 회원 관련 테스트 전체(65개: `MemberTest` 31 + `MemberServiceTest` 17 + `MemberSelfControllerTest` 6 + `AdminMemberControllerTest` 7 + `UserResponseDTOTest` 4) 리팩토링 전후 동일하게 통과, PMD 통과, 전체 회귀 테스트 통과
