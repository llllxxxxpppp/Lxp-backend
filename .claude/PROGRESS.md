...
- `member/model/entity/Member` — 회원 애그리거트 루트
...
- 프로필 수정은 INSTRUCTOR 역할만 가능

**팩토리 메서드**
- `Member.createMember(email, encodedPassword)` — 일반 회원 생성
- `Member.createInstructor(email, encodedPassword, name, profileImageUrl, introduction)` — 강사 생성
- `Member.createAdmin(email, encodedPassword)` — 어드민 생성

**테스트 현황**: InstructorProfileTest(6), MemberTest(23) — 전체 통과, PMD 통과
